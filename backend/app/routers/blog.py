...
def list_posts(
    ...
    if category:
        query = query.filter(func.lower(models.BlogPost.category) == category.lower())
    if tag:
        query = query.filter(
            text("blog_posts.tags @> :tag_json").bindparams(tag_json=json.dumps([tag]))
        )
    if search:
...
def featured_posts(db: Session = Depends(get_db)):
    return (
        ...
        .filter(models.BlogPost.is_published == True, models.BlogPost.is_featured == True)
...
def list_tags(db: Session = Depends(get_db)):
    posts = db.query(models.BlogPost.tags).filter(models.BlogPost.is_published == True).all()
    all_tags = set()
    for (tags,) in posts:
        if tags:
...
def list_all_posts(db: Session = Depends(get_db), _: models.AdminUser = Depends(get_current_user)):
...
def get_post_by_id(post_id: int, db: Session = Depends(get_db), _: models.AdminUser = Depends(get_current_user)):
