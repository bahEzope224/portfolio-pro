...
def list_posts(
    ...
    """Liste les articles publiés avec filtres optionnels."""
    ...
    if tag:
        # Recherche dans le JSON array des tags
        query = query.filter(
            models.BlogPost.tags.contains([tag])
        )
    if search:
...
def featured_posts(db: Session = Depends(get_db)):
    """Retourne les articles mis en avant (pour la homepage)."""
    return (
...
def list_categories(db: Session = Depends(get_db)):
    """Retourne toutes les catégories des articles publiés."""
    rows = (
...
def list_tags(db: Session = Depends(get_db)):
    """Retourne tous les tags uniques des articles publiés."""
    posts = (
...
def get_post(slug: str, db: Session = Depends(get_db)):
    """Retourne un article complet et incrémente son compteur de vues."""
...
def list_all_posts(
    ...
    """Liste TOUS les articles (publiés + brouillons) pour l'admin."""
    return (
        db.query(models.BlogPost)
        .order_by(models.BlogPost.updated_at.desc())
        .all()
    )
